...
class ModelKualitasUdara:
    ...
    def estimasi_parameter(self):
        k_vals = []
        for i in range(1, self.n_data - 1):
            dP = (self.P_aktual[i+1] - self.P_aktual[i-1]) / (2 * self.h)
            if dP < 0 and self.P_aktual[i] > 0:
                k_vals.append(-dP / self.P_aktual[i])

        k_estimasi = float(np.median(k_vals))
        E_estimasi = k_estimasi * float(np.mean(self.P_aktual))

        print(f"\n--- ESTIMASI PARAMETER OTOMATIS ---")
        print(f"k (laju dispersi)  = {k_estimasi:.4f} /jam")
        print(f"E (laju emisi)     = {E_estimasi:.4f} µg/m³/jam")
        print(f"Equilibrium E/k    = {E_estimasi/k_estimasi:.2f} µg/m³  (≈ mean PM2.5 aktual)")

        return E_estimasi, k_estimasi
...
if __name__ == "__main__":
    ...
    # REVISI: Menggunakan fungsi estimasi otomatis untuk mendapatkan nilai E dan k
    E_estimasi, k_estimasi = model.estimasi_parameter()

    # Menghitung simulasi menggunakan parameter hasil estimasi otomatis
    ...
    ax1.plot(model.waktu, hasil_simulasi_rk4, color='red', linestyle='dashed', linewidth=1.5, 
             label=f'Simulasi RK4 PM2.5 (E={E_estimasi:.2f}, k={k_estimasi:.2f})')
    ...
    # Sumbu Y kedua untuk CO (karena skala satuannya berbeda)
    ax2 = ax1.twinx()
    ...
    # Menggabungkan legend dari kedua sumbu Y
